"""Final classification with all patterns correct"""
import re, pickle
...
for i, para in enumerate(doc.paragraphs):
    ...
    if style.startswith('toc'):
        classifications.append((i, 'toc', text))
        continue
    ...
    if text == '目录' or text == '目  录':
        classifications.append((i, 'toc_title', text))
        continue
    ...
    if text in ['附录', '附  录']:
        in_ack = False
        in_appendix = True
        classifications.append((i, 'appendix_title', text))
        continue
    
    if re.match(r'^附录[A-Z]\s', text):
        in_ack = False
        in_appendix = True
        classifications.append((i, 'appendix_section', text))
        continue
    ...
    # Appendix body
    if in_appendix:
        classifications.append((i, 'appendix_body', text))
        continue
    ...
    # Section headings
    if re.match(r'^\d+\.\d+\.\d+\s', text) and len(text) < 50 and not re.search(r'[。，；]', text):
        classifications.append((i, 'h3', text))
        continue
    if re.match(r'^\d+\.\d+\s', text) and len(text) < 40 and not re.search(r'[。，；]', text):
